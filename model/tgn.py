...
class TGN(torch.nn.Module):
  ...
  def compute_edge_probabilities(self, source_nodes, destination_nodes, negative_nodes, edge_times,
    ...
    if not self.use_inner_product:
      ...
      score = torch.sum(torch.mul(torch.cat([source_node_embedding, source_node_embedding], dim=0), torch.cat([destination_node_embedding,
                                           negative_node_embedding])), dim=1)
